  ...
  return (
    <div className="flex justify-center items-center gap-4 relative">
      ...
      <motion.div
        ...
        whileTap={{ scale: 0.9 }}
        className="px-4 py-4 rounded-md flex items-center justify-center bg-secondary cursor-pointer text-emerald-500"
      >
      ...
      {isMenu && (
        <motion.div className="bg-secondary absolute top-16 right-0 px-4 py-3 rounded-xl shadow-md z-10 flex flex-col items-start justify-start gap-4 min-w-[225px]">
          {Menus &&
            Menus.map((e) => (
              <Link
                to={e.url}
                key={e.id}
                className="text-primaryText text-lg hover:bg-[rgba(256,256,256,0.05)] px-2 py-1 w-full rounded-md text-gray-300"
              >
          ...
          <motion.p
            ...
            whileTap={{ scale: 0.9 }}
            className="text-primaryText text-lg hover:bg-[rgba(256,256,256,0.05)] px-2 py-1 w-full rounded-md cursor-pointer text-red-400"
          >
            Sign Out
          </motion.p>
        </motion.div>
